...
1. **Generate** (`scripts/generate-content.ts`, nightly GH Action `generate-content.yml` at 17:00 UTC): reads the live app's public `/api/inventory`, writes validated JSON to `content/generated/<category>/<timestamp>.json`, commits to `main` — which itself triggers a deploy. Provider: Gemini first, Groq (llama) fallback (`src/lib/ai-provider.ts`); the Gemini key is paid (as of 2026-07-19) but still finite quota, which is why runs cap categories per night (`TOPUP_MAX_CATEGORIES`) rather than going uncapped.
