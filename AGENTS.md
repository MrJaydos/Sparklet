...
1. **Generate** (`scripts/generate-content.ts`, nightly GH Action `generate-content.yml` at 17:00 UTC): reads the live app's public `/api/inventory`, writes validated JSON to `content/generated/<category>/<timestamp>.json`, commits to `main` — which itself triggers a deploy. Provider: Gemini first, Groq (llama) fallback (`src/lib/ai-provider.ts`); the Gemini key is paid (as of 2026-07-19) but still finite quota, which is why runs cap categories per night (`TOPUP_MAX_CATEGORIES`) rather than going uncapped. `--top-up` (the scheduled mode) runs on Gemini **batch mode** — half price, async — instead of per-category calls: each run collects the previous night's batch (writing its cards) then submits one new batch for tonight's low categories, skipping submission if one's still in flight. That gives a ~24h lag between a category going low and cards landing, which is fine for a background bank; `--all`/`--category` (a human waiting on output) stay synchronous.
