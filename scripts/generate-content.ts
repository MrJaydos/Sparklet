...
- "sources": 2-3 REAL sources as {"title", "publisher", "url"}. CRITICAL: only cite URLs you are certain exist. NEVER invent a plausible-looking URL. Every URL is automatically checked and the card is discarded if any link is dead.
  - SOURCE DIVERSITY: at least one source MUST NOT be Wikipedia. A card whose only support is a Wikipedia article is a paraphrase of Wikipedia, not independently sourced — prefer .gov/.edu pages, PubMed/PMC, NASA/NOAA/USGS/ESA, major museums and libraries, WHO/UN/World Bank, or open-access journals, and use Wikipedia as the *supporting* citation rather than the only one. If you genuinely cannot name a second source URL you are certain exists, pick a different fact you can source properly.
  - "publisher" must be the organisation that actually publishes THAT EXACT URL — for an en.wikipedia.org link the publisher is "Wikipedia", never the institution the article happens to be about. (Attributing a Wikipedia link to "NASA" or a museum is a fabricated citation. The importer overwrites this field from the URL's host, so there is nothing to gain by embellishing it.)
  - NEVER cite a source that requires a paid subscription or institutional login to read in full — no NYT/WSJ/Bloomberg/FT/Economist/Washington Post, and no academic-journal publisher pages (ScienceDirect, SpringerLink, SAGE Journals, Wiley Online Library, JSTOR, IEEE Xplore, NEJM, The Lancet, APA PsycNet, etc.). A reader must be able to open and read the whole source for free.
